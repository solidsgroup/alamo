...
#   GPU_NODE_CPUS=24   CPU cores on one NOVA GPU node; split across GPU ranks.
...
emit() {
    ...
    if [[ "$kind" == "gpu" ]]; then
        ...
        sbatch_opts+=(--nodes="${nodes}" --gres="gpu:${GPU_TYPE}:${gpus_on_node}" --ntasks="${ngpus}" --ntasks-per-node="${tasks_per_node}" --cpus-per-task="${cpus_per_task}")
    else
...
: > "$OUT"
{
    ...
    echo "# MODE=$MODE GPU_TYPE=$GPU_TYPE GPUS_PER_NODE=$GPUS_PER_NODE GPU_NODE_CPUS=$GPU_NODE_CPUS SIZES='$SIZES' GPUS='$GPUS'"
