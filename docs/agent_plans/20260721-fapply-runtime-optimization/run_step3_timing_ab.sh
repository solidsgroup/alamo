...
if [[ "$REGIME" == 2d ]]; then
  CASE=2d_conservative_2x2
  INPUT=tests/ElasticSoftVoid/input
  BASE_FAST="$ART/baseline/bin/alamo_gpu-2d-fast-sm86-baseline"
  BASE_PROFILE="$ART/baseline/bin/alamo_gpu-2d-profile-fast-sm86-baseline"
  CAND_FAST="$CAND/isolated/bin/alamo_gpu-2d-fast-sm86-step3-isolated"
  CAND_PROFILE="$CAND/isolated/bin/alamo_gpu-2d-profile-fast-sm86-step3-isolated"
  COMMON=(
  ...
  BASE_FAST="$ART/baseline/bin/alamo_gpu-3d-fast-sm86-baseline"
  BASE_PROFILE="$ART/baseline/bin/alamo_gpu-3d-profile-fast-sm86-baseline"
  CAND_FAST="$CAND/isolated/bin/alamo_gpu-3d-fast-sm86-step3-isolated"
  CAND_PROFILE="$CAND/isolated/bin/alamo_gpu-3d-profile-fast-sm86-step3-isolated"
  COMMON=(
...
test ! -e "$OUT/$CASE"
sha256sum "$BASE_FAST" "$BASE_PROFILE" "$CAND_FAST" "$CAND_PROFILE" "$INPUT"

run_one () {
  local arm="$1" mode="$2" label="$3" binary="$4"
  local dir="$OUT/$CASE/$arm/$mode/$label"
  test ! -e "$dir"
  mkdir -p "$dir"
  ...
  printf '%s %s %s ' "$arm" "$mode" "$label"
  tr -d '\n' <"$dir/wall_seconds.txt"
  printf '\n'
}

for mode in unprofiled profiled; do
  if [[ "$mode" == unprofiled ]]; then
    base_binary="$BASE_FAST"
    cand_binary="$CAND_FAST"
  else
    base_binary="$BASE_PROFILE"
    cand_binary="$CAND_PROFILE"
  fi
  run_one baseline "$mode" warmup "$base_binary"
  run_one candidate "$mode" warmup "$cand_binary"
  for rep in 1 2 3 4 5; do
    if (( rep % 2 == 1 )); then
      run_one baseline "$mode" "rep$rep" "$base_binary"
      run_one candidate "$mode" "rep$rep" "$cand_binary"
    else
      run_one candidate "$mode" "rep$rep" "$cand_binary"
      run_one baseline "$mode" "rep$rep" "$base_binary"
    fi
...
git diff --check -- src/Operator/Elastic.cpp src/Operator/Elastic.H \
  src/Test/Operator/Elastic.H src/test.cc
set +x
echo "STEP3_TIMING_${REGIME^^}_PASS"
