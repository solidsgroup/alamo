LowMach elastic inclusion (THIN-INTERFACE variant): AP in an HTPB block, sharper eta transition
=================================================================================================

**Diffuse-interface-width sensitivity study.** This is a variant of
``tests/LowMachElasticInclusion`` that halves the diffuse ``eta`` interface
width, ``w``: ``4.0e-5 -> 2.0e-5`` m, with the mesh spacing ``dx`` halved to
match (so the interface still spans the same ~2.1 cells physically -- see
``input``). Domain size and material moduli are unchanged from the
baseline test. The analytic reference assumes a perfectly sharp interface
at ``r=a``, while the simulation's ``eta`` field transitions smoothly over
a band of width ``~w`` around it (``test`` excludes a ``3*w`` band around
that transition from the comparison, since the two descriptions
necessarily disagree there); halving ``w`` should shrink both that excluded
band and the disagreement it's hiding, so this test checks whether it
correspondingly shrinks the RMS error against the baseline test's. See
that test's Readme for the full derivation, reproduced below with this
variant's numbers substituted in.
...
1. Interior stress: ``stress_xx`` uniform and equal to ``sigma_xx_in``,
   ...
   ``w=4.0e-5`` -- see below for this variant's achieved error at
   ``w=2.0e-5``).
2. Full-profile **absolute** RMS error of ``disp_x``, ``stress_xx``,
   ...
   ``w=4.0e-5``; this is the error this interface-sharpening variant is
   checking for improvement in).
