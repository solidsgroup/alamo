#!/usr/bin/env python3
"""
Generate the analytical reference data for the LowMachElasticInclusionThinInterface
test -- the THIN-INTERFACE variant of tests/LowMachElasticInclusion (see this
directory's Readme.rst). The closed-form solution below is IDENTICAL to the
baseline test's (it assumes a perfectly sharp interface at r=a and depends
only on the material moduli, inclusion radius `a`, and applied pressure `P`
-- none of which change here); only `input`'s diffuse interface width w
changes (halved, with dx halved to match), and correspondingly `test`'s
excluded interface band (3*w) shrinks with it. This checks whether
sharpening the diffuse eta transition shrinks this test's RMS error against
the sharp-interface analytic solution, as expected.
...
w = 2.0e-5                    # diffuse interface width (must match `input`)
