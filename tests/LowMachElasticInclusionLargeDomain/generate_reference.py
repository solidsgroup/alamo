#!/usr/bin/env python3
"""
Generate the analytical reference data for the LowMachElasticInclusionLargeDomain
test -- the LARGE-DOMAIN variant of tests/LowMachElasticInclusion (see this
directory's Readme.rst). The closed-form solution below is IDENTICAL to the
baseline test's (it depends only on the material moduli, inclusion radius
`a`, and applied pressure `P` -- none of which change here); only `input`'s
domain size changes, from a=1/10 of the block half-width there to a=1/30
here, to check whether widening the domain (holding dx fixed) shrinks this
test's RMS error, as the analytic solution's neglected Saint-Venant/image
correction (O((a/L)^2), see Step 2 below) predicts it should.
...
square HTPB block that fills the domain width and extends from y=-4.5e-3 to
y=ytop=4.5e-3 (3x wider than the baseline test's y=-1.5e-3 to y=1.5e-3); a
Gas layer above y=ytop delivers `elastic.apply_fluid_
...
block is 30x the inclusion radius on every side here (vs. 10x in the
baseline test) -- Saint-Venant/image corrections are O((a/L)^2), so this
domain widening should shrink that neglected term by ~9x relative to the
baseline test's, negligible next to the ~5-8% error this test
