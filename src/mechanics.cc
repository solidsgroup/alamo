...
int main(int argc, char *argv[]) {
    ...
    if (program == "mechanics") {
        ...
        if (model == "linear.isotropic")
            integrator =
                new Integrator::Mechanics<Model::Solid::Linear::Isotropic>(pp);
        else if (model == "linear.TransverselyIsotropic")
            integrator = new Integrator::Mechanics<
                Model::Solid::Linear::TransverselyIsotropic>(pp);
        else if (model == "linear.cubic")
            integrator =
                new Integrator::Mechanics<Model::Solid::Linear::Cubic>(pp);
        else if (model == "affine.cubic")
            integrator =
                new Integrator::Mechanics<Model::Solid::Affine::Cubic>(pp);
        else if (model == "affine.hexagonal")
            integrator =
                new Integrator::Mechanics<Model::Solid::Affine::Hexagonal>(pp);
        else if (model == "affine.isotropic")
            integrator =
                new Integrator::Mechanics<Model::Solid::Affine::Isotropic>(pp);
        else if (model == "linear.laplacian")
            integrator =
                new Integrator::Mechanics<Model::Solid::Linear::Laplacian>(pp);
        else if (model == "finite.neohookean")
            integrator =
                new Integrator::Mechanics<Model::Solid::Finite::NeoHookean>(pp);
        else if (model == "finite.neohookeanpre")
            integrator = new Integrator::Mechanics<
                Model::Solid::Finite::NeoHookeanPredeformed>(pp);
        else if (model == "finite.adhesion")
            integrator =
                new Integrator::Mechanics<Model::Solid::Finite::Adhesion>(pp);
        else if (model == "finite.pseudolinear.cubic")
            integrator = new Integrator::Mechanics<
                Model::Solid::Finite::PseudoLinear::Cubic>(pp);
        else if (model == "finite.pseudoaffine.cubic")
            integrator = new Integrator::Mechanics<
                Model::Solid::Finite::PseudoAffine::Cubic>(pp);
        else if (model == "affine.j2")
            integrator =
                new Integrator::Mechanics<Model::Solid::Affine::J2>(pp);
        else if (model == "finite.crystalplastic")
            integrator =
                new Integrator::Mechanics<Model::Solid::Finite::CrystalPlastic>(
                    pp);
        else
