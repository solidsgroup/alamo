...
MyTest::solveABecLaplacian ()
{
    ...
    if (composite_solve)
    {
	    ...
	    for (amrex::MFIter mfi(solution[ilev], true); mfi.isValid(); ++mfi)
	      {
		const amrex::Box& box = mfi.tilebox();
		amrex::BaseFab<amrex::Real> &sol_box = solution[ilev][mfi];
		amrex::Box domain(geom[ilev].Domain());

		for (int i = box.loVect()[0] - solution[ilev].nGrow(); i<=box.hiVect()[0] + solution[ilev].nGrow(); i++)
		  for (int j = box.loVect()[1] - solution[ilev].nGrow(); j<=box.hiVect()[1] + solution[ilev].nGrow(); j++)
		    { 
		      if (j > domain.hiVect()[1]) // Top boundary
			sol_box(amrex::IntVect(i,j)) = 1.0;
		    }
	      }
