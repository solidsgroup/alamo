...
void
Constant::FillBoundary (amrex::BaseFab<Set::Vector> &a_in,
                        const amrex::Box &a_box,
                        int ngrow, int /*dcomp*/, int /*ncomp*/, amrex::Real time,
                        Orientation face, const amrex::Mask * /*mask*/)
{
    ...
    Util::Assert(INFO,TEST(a_in.nComp() == 1));
    Util::Assert(INFO,TEST((int)m_ncomp == AMREX_SPACEDIM));
...
{
    return amrex::Periodicity(
        amrex::IntVect( AMREX_D_DECL(
                            BC<Set::Scalar>::m_geom.Domain().length(0) * BCUtil::IsPeriodic(m_bc_type[Face::XLO][0]),
