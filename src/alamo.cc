...
int main(int argc, char *argv[])
{
    ...
    pp.query_validate("alamo.program", program, { "microstructure", "flame", "heat", "dendrite", "allencahn", "cahnhilliard", "pfc", "agglomeration" });
    srand(2);
    ...
    if (program == "microstructure")
    {
        ...
        pp.query_validate("alamo.program.microstructure.model", model, { "affine.cubic", "affine.hexagonal", "finite.pseudoaffine.cubic" });
        ...
        pp.select_only<Integrator::AllenCahn>(integrator);
    else if (program == "cahnhilliard")
        pp.select_only<Integrator::CahnHilliard>(integrator);
    else if (program == "pfc")
        pp.select_only<Integrator::PFC>(integrator);
    else if (program == "agglomeration")
        pp.select_only<Integrator::Agglomeration>(integrator);
    else
