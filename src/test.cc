...
// 		test.Define({AMREX_D_DECL(32,16,8)});
...
// 	}
	...
	Util::Test::Message("Elastic Operator Trig Test 32x32");
	{
		...
		// test.Define({AMREX_D_DECL(32,16,8)},1);
		// subfailed += Util::Test::SubMessage("1 level,  Component 0, period=1",test.TrigTest(3,0,1));
		// test.Define(32,2);
		// subfailed += Util::Test::SubMessage("2 levels, Reflux test",          test.RefluxTest(0));
		// subfailed += Util::Test::SubMessage("2 levels, Component 0, period=1",test.TrigTest(3,0,1));
		test.Define({96,24,8},3,1);
		subfailed += Util::Test::SubMessage("3 levels, Reflux test",          test.RefluxTest(0));
		subfailed += Util::Test::SubMessage("3 levels, Component 0, period=1",test.TrigTest(3,0,1,"testoutput"));
		failed += Util::Test::SubFinalMessage(subfailed);
	}
