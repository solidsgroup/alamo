...
SolverCapabilities::MethodSupport 
ElastoPlasticCapabilities::supportsFluxReconstruction(FluxReconstructionType method) const {
    switch(method) {
        case FluxReconstructionType::FirstOrder:
        case FluxReconstructionType::WENO:
            return MethodSupport::Supported();
...
SolverCapabilities::MethodSupport 
ElastoPlasticCapabilities::supportsFluxScheme(FluxScheme scheme) const {
    switch(scheme) {
        case FluxScheme::LocalLaxFriedrichs:
        case FluxScheme::HLLC:
            return MethodSupport::Supported();
...
SolverCapabilities::MethodSupport 
ElastoPlasticCapabilities::supportsTimeSteppingScheme(TimeSteppingSchemeType scheme) const {
    switch(scheme) {
        case TimeSteppingSchemeType::ForwardEuler:
        case TimeSteppingSchemeType::RK3:
            return MethodSupport::Supported();
...
SolverCapabilities::MethodSupport 
ElastoPlasticCapabilities::supportsReconstructionMode(ReconstructionMode mode) const {
    switch(mode) {
        case ReconstructionMode::Primitive:
        case ReconstructionMode::Conservative:
        case ReconstructionMode::Characteristic:
...
SolverCapabilities::MethodSupport 
ElastoPlasticCapabilities::supportsWenoVariant(WenoVariant variant) const {
    switch(variant) {
        case WenoVariant::WENOJS5:
        case WenoVariant::WENOZ5:
        case WenoVariant::WENOJS3:
...
SolverCapabilities::MethodValidationResult 
ElastoPlasticCapabilities::validateMethodCombination(
    FluxReconstructionType fluxReconstruction [[maybe_unused]],
    FluxScheme fluxScheme,
    TimeSteppingSchemeType timeSteppingScheme [[maybe_unused]],
    ReconstructionMode reconstructionMode,
...
SolverCapabilities::DefaultConfiguration 
ElastoPlasticCapabilities::getDefaultConfiguration() const {
    return {
        FluxReconstructionType::WENO,
        FluxScheme::HLLC,
        TimeSteppingSchemeType::RK3,
...
std::shared_ptr<GenericVariableAccessor>
ElastoPlasticCapabilities::createVariableAccessor(
    ReconstructionMode mode,
    int numGhostCells) const 
{
    return std::make_shared<ElastoPlasticVariableAccessor>(
        numGhostCells, mode);
}
