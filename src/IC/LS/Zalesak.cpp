...
void Zalesak::Parse(Zalesak& value, IO::ParmParse& pp) {
    pp_query("radius", value.radius);  // radius of the circle the slot is cut from
    pp_queryarr("center", value.center);  // center vector coordinate of disk
    pp_queryarr("slot_center", value.slot_center);  // the center coordinate of the slot rectangle that is cut out
    pp_queryarr("slot_size", value.slot_size); // lengths iin x, y, and z planes of rectangle that is cut out
    pp_query("fillet_radius", value.fillet_radius); // the radius of the fillet used to smooth slot
    std::string type_str;
    pp_query("type", type_str); // the plane of the circle the disk lies on
