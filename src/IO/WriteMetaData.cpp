...
void WriteMetaData(std::string plot_file, Status status, int per)
{
    if (amrex::ParallelDescriptor::IOProcessor())
    {
        ...
        metadatafile << "Git_development_commit_hash = " << METADATA_DEVELOPMENT_GITHASH << std::endl;
        ...
        if (status == Status::Running)
            metadatafile << "Status = Running";
        else if (status == Status::Complete)
            metadatafile << "Status = Complete";
        else if (status == Status::Abort)
            metadatafile << "Status = Abort";
        else if (status == Status::Segfault)
            metadatafile << "Status = Segfault";
        else if (status == Status::Interrupt)
            metadatafile << "Status = Interrupt";

        if (percent > 0)
            metadatafile << " (" << percent << "%)";
        metadatafile << std::endl;
        ...
        {
            std::ifstream src(GIT_DEVELOPMENT_DIFF_PATCH_OUTPUT, std::ios::binary);
            if (src.is_open())
            {
                std::ofstream dst(plot_file + "/development-diff.patch", std::ios::binary);
                dst << src.rdbuf();
                src.close();
                dst.close();
            }
            ...
            {
                Util::Warning(INFO, "Could not open ", GIT_DEVELOPMENT_DIFF_PATCH_OUTPUT);
            }
        }
