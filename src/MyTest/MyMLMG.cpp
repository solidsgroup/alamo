...
MLMG::oneIter (int iter)
{
    ...
    for (int alev = finest_amr_lev; alev > 0; --alev)
    {
        ...
        MultiFab::Add(*sol[alev], *cor[alev][0], 0, 0, sol[alev]->nComp(), 0);
    ...
    // coarest amr level
    {
        ...
        MultiFab::Add(*sol[0], *cor[0][0], 0, 0, sol[0]->nComp(), 0);
    }
    ...
    for (int alev = 1; alev <= finest_amr_lev; ++alev)
    {
        ...
        MultiFab::Add(*sol[alev], *cor[alev][0], 0, 0, sol[0]->nComp(), 0);

        if (alev != finest_amr_lev) {
	  MultiFab::Add(*cor_hold[alev][0], *cor[alev][0], 0, 0, cor_hold[alev][0]->nComp(), 0);
        }
        ...
        if (alev != finest_amr_lev) {
	  MultiFab::Add(*cor[alev][0], *cor_hold[alev][0], 0, 0, cor[alev][0]->nComp(), 0);
        }
    }
...
MLMG::mgFcycle ()
{
    ...
    {
      amrex::average_down(res[amrlev][mglev-1], res[amrlev][mglev], 0, res[amrlev][mglev-1].nComp(), ratio);
    }
    ...
    for (int mglev = mg_bottom_lev-1; mglev >= 0; --mglev)
    {
        ...
        MultiFab::Copy(res[amrlev][mglev], rescor[amrlev][mglev], 0,0,res[amrlev][mglev].nComp(),0);

        // save cor; do v-cycle; add the saved to cor
        std::swap(cor[amrlev][mglev], cor_hold[amrlev][mglev]);
        mgVcycle(amrlev, mglev);
        MultiFab::Add(*cor[amrlev][mglev], *cor_hold[amrlev][mglev], 0, 0, cor[amrlev][mglev]->nComp(), 0);
    }
}
...
MLMG::interpCorrection (int alev)
{
    ...
    MultiFab cfine(ba, fine_cor.DistributionMap(), crse_cor.nComp(), ng);
    cfine.setVal(0.0);
    cfine.ParallelCopy(crse_cor, 0, 0, crse_cor.nComp(), 0, ng, crse_geom.periodicity());
...
#endif
        {
            ...
            for (MFIter mfi(fine_cor, true); mfi.isValid(); ++mfi)
            {
                ...
                amrex_mlmg_lin_nd_interp(BL_TO_FORTRAN_BOX(cbx),
                                         ...
                                         BL_TO_FORTRAN_ANYD(cfine[mfi]));
                fine_cor[mfi].copy(tmpfab, fbx, 0, fbx, 0, tmpfab.nComp());
            }
        }
    }
}
...
MLMG::actualBottomSolve ()
{
    ...
    else
    {
        ...
        {
	  raii_b.define(b.boxArray(), b.DistributionMap(), b.nComp(), b.nGrow());
	  MultiFab::Copy(raii_b,b,0,0,b.nComp(),b.nGrow());
            bottom_b = &raii_b;
...
MLMG::prepareForSolve (const Vector<MultiFab*>& a_sol, const Vector<MultiFab const*>& a_rhs)
{
    ...
    {
      rhs[alev].define(a_rhs[alev]->boxArray(), a_rhs[alev]->DistributionMap(), a_rhs[alev]->nComp(), 0);
      MultiFab::Copy(rhs[alev], *a_rhs[alev], 0, 0, rhs[alev].nComp(), 0);
...
MLMG::prepareForNSolve ()
{
    ...
    ns_sol.reset(new MultiFab(ba, dm, sol[0]->nComp(), 1));
    ns_rhs.reset(new MultiFab(ba, dm, sol[0]->nComp(), 0));
    ns_sol->setVal(0.0);
    ns_rhs->setVal(0.0);
                    ...
                    const Vector<MultiFab const*>& a_rhs)
{
    ...
    for (int alev = finest_amr_lev; alev >= 0; --alev) {
        ...
        MultiFab::Copy(rhstmp, *prhs, 0, 0, rhstmp.nComp(), 0);
