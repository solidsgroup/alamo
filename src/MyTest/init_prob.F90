...
module init_prob_module
  ...
  implicit none
  private
  public :: actual_init_abeclap

contains


  subroutine actual_init_abeclap ( &
       lo, hi, &           ! Box
       glo, ghi, &         ! Box
       rhs, rlo, rhi, &    ! Right-hand side of the equation (source term)
       exact, elo, ehi, &  ! Exact solution
       alpha, alo, ahi, &  ! Alpha coefficient
       beta, blo, bhi, &   ! Beta coefficient
       a, b, &             ! Scalars
       prob_lo, prob_hi, & ! Geometry
       dx) &               ! Cell size
