									...
									const amrex::IntVect loc, const int amrlev, const int mglev, const MFIter &mfi) const
{
	...
	std::array<amrex::Real,3> dMdE = {0.0,0.0,0.0}; //assuming num_eta = 3 here.
	if (m != n) return dMdE;
								...
								const int amrlev, const int mglev, const MFIter &mfi) const
{
	...
	std::array<amrex::Real,3> dNdE = {0.0,0.0,0.0}; //assuming num_eta = 3 here.
								...
								for (int p=0; p<AMREX_SPACEDIM; p++)
									for (int q=0; q<AMREX_SPACEDIM; q++)
										for (int r=0; r<AMREX_SPACEDIM; r++)
											for (int s=0; s<AMREX_SPACEDIM; s++)
											{
												std::array<amrex::Real,3> dMdE1 = dMdEta(p,q,i,j,m,amrlev,mglev,mfi);
												std::array<amrex::Real,3> dMdE2 = dMdEta(r,s,k,l,m,amrlev,mglev,mfi);
