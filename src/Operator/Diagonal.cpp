        ...
        ) const
{
    ...
    for (MFIter mfi(f, true); mfi.isValid(); ++mfi)
    {
        ...
        if(ufab.contains_inf<amrex::RunOn::Host>()) Util::Abort(INFO, "Inf in ufab [before update]");
        if(ufab.contains_nan<amrex::RunOn::Host>()) Util::Abort(INFO, "Nan in ufab [before update]");
    }
    ...
    {
        amrex::FArrayBox       &ffab    = f[mfi];
        if(ffab.contains_inf<amrex::RunOn::Host>()) Util::Abort(INFO, "Inf in ffab [after update]");
        if(ffab.contains_nan<amrex::RunOn::Host>()) Util::Abort(INFO, "Nan in ffab [after update]");
    }
}
