...
>>>>>>> 6c1f6c815d610407b90dc9d631ab8906f33e70d3
...
<<<<<<< HEAD
            ...
            E_new(i, j, k) =
                ...
                -  0.5 * gamma/(gamma - 1.0) * (v(i+1,j,k,0) * p(i+1,j,k) * eta(i+1, j, k) - v(i-1,j,k,0) * p(i-1,j,k) * eta(i-1, j, k)) / DX[0]) * dt
            ...
            rho_new(i, j, k) =
                ...
                eta(i, j, k) * rho(i, j, k)
                + ((flux_xlo.etaMass - flux_xhi.etaMass) / DX[0]) * dt
            ...
            M_new(i, j, k, 0) =
                ...
                - 0.5 * (p(i+1,j,k) - p(i-1,j,k)) / DX[0] * eta(i, j, k) * eta(i, j, k)) * dt
...
=======
            ...
            etaE_new(i, j, k) =
                ...
                + 0.5 * gamma * ((v(i + 1, j, k, 0) * etap(i + 1, j, k) - v(i - 1, j, k, 0) * etap(i - 1, j, k)) - etap(i, j, k) * v(i, j, k, 0) / (eta_cell + small) * (eta_xhi - eta_xlo)) / DX[0] * dt
