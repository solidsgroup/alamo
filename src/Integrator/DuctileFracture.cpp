	...
	Integrator()
{
	...
	RegisterNodalFab (m_strain_old,				number_of_stress_components,	number_of_ghost_nodes,	"strain_old",	false);
	...
	RegisterNodalFab (m_stressdev,				number_of_stress_components,	number_of_ghost_nodes,	"stress_dev",	false);
	...
	RegisterNodalFab (m_energy_pristine_old,	1,								number_of_ghost_nodes,	"energyPOld",	false);
	...
	RegisterNodalFab (m_strain_p_old,			number_of_stress_components,	number_of_ghost_nodes,	"strainp_old",	false);
	RegisterNodalFab (m_beta,					number_of_stress_components,	number_of_ghost_nodes,	"beta",			true);
	RegisterNodalFab (m_alpha,					1,								number_of_ghost_nodes,	"eqv_strain",		true);
...
DuctileFracture::TimeStepBegin(amrex::Real /*time*/, int /*iter*/)
{
	...
	// Let's first update the model fab
	material.model.resize(nlevels);
	...
	Set::Scalar epsp_rel_err = 1.0;
	int max_iter_p = 25, curr_iter_p = 1;

	while (curr_iter_p < max_iter_p && epsp_rel_err < tol_ep)
	{
		// Let's swap old and new fields and update the fields
		for (int ilev = 0; ilev < nlevels; ++ilev)
		{
		...
		// Next let's set plastic strains in the model
		for (int ilev = 0; ilev < nlevels; ++ilev)
		{
		...
		// Mode-I fracture. We can adopt this later for Mode-II and Mode-III
		...
		// Let's do a plastic update now.
		for (int ilev = 0; ilev < nlevels; ++ilev)
		{
			// Step 1: compute stresses, strains and energies
			elastic_op.Strain(ilev,*m_strain[ilev],*m_disp[ilev]);
			elastic_op.Stress(ilev,*m_stress[ilev],*m_disp[ilev]);

			// Step 2: Evolve plastic strains
			for (amrex::MFIter mfi(*m_strain[ilev],true); mfi.isValid(); ++mfi)
			{
				const amrex::Box& box = mfi.validbox();
				amrex::Array4<Set::Scalar>					const& sig_box 			= (*m_stress[ilev]).array(mfi);
				amrex::Array4<const Set::Scalar>			const& eps_box 			= (*m_strain[ilev]).array(mfi);
				amrex::Array4<const Set::Scalar>			const& eps_old_box 		= (*m_strain_old[ilev]).array(mfi);
				amrex::Array4<Set::Scalar>					const& strainp_box 		= (*m_strain_p[ilev]).array(mfi);
				amrex::Array4<Set::Scalar>					const& beta_box 		= (*m_beta[ilev]).array(mfi);
				amrex::Array4<Set::Scalar>					const& alpha_box 		= (*m_alpha[ilev]).array(mfi);
				amrex::Array4<ductile_fracture_model_type>	const& model_box 		= material.model[ilev]->array(mfi);

				amrex::ParallelFor (box,[=] AMREX_GPU_DEVICE(int i, int j, int k){
					Set::Matrix sig = Numeric::FieldToMatrix(sig_box,i,j,k);
					Set::Matrix eps = Numeric::FieldToMatrix(eps_box,i,j,k);
					Set::Matrix eps_old = Numeric::FieldToMatrix(eps_old_box,i,j,k);
					Set::Matrix epsdev = eps - (1.0/((double)AMREX_SPACEDIM))*eps.trace()*Set::Matrix::Identity();
					Set::Matrix epsolddev = eps_old - (1.0/((double)AMREX_SPACEDIM))*eps_old.trace()*Set::Matrix::Identity();

					model_box(i,j,k,0).EvolvePlasticStrain(sig,epsdev-epsolddev,dt[ilev]);
		...
		// Now we compute error norms
		ep_err_norm = 0.; ep_norm = 0.;

		for (int ilev = 0; ilev < nlevels; ++ilev)
		{
			const amrex::Real* DX = geom[ilev].CellSize();
			for (amrex::MFIter mfi(*m_strain_p[ilev],true); mfi.isValid(); ++mfi)
			{
				const amrex::Box& box = mfi.validbox();
				amrex::Array4<const Set::Scalar> const &ep_old 	= (*m_strain_p[ilev]).array(mfi);
				amrex::Array4<const Set::Scalar> const &ep_new 	= (*m_strain_p_old[ilev]).array(mfi);
				amrex::Array4<const Set::Scalar> const &c_old 	= (*m_c_old[ilev]).array(mfi);
				...
				amrex::ParallelFor (box,[=] AMREX_GPU_DEVICE(int i, int j, int k){
					...
					ep_norm *= crack.boundary->g_phi(c_old(i,j,k,0),0);
					ep_err_norm *= crack.boundary->g_phi(c_old(i,j,k,0),0);
				});
			}
		}
		epsp_rel_err = ep_err_norm/ep_norm;
		Util::Message(INFO,"Iteration ", curr_iter_p, " complete. Relative error = ", epsp_rel_err);
		curr_iter_p ++;
	}

	// Now once everything is done, let's finalize stuff
	for (int ilev = 0; ilev < nlevels; ++ilev)
	{
		for (amrex::MFIter mfi(*m_strain[ilev],true); mfi.isValid(); ++mfi)
		{
			...
			amrex::Array4<const Set::Scalar>			const& eps_box 			= (*m_strain[ilev]).array(mfi);
			amrex::Array4<ductile_fracture_model_type>	const& model_box 		= material.model[ilev]->array(mfi);
			amrex::Array4<Set::Scalar>					const& energy_box		= (*m_energy_pristine[ilev]).array(mfi);
			amrex::Array4<const Set::Scalar>			const& energy_old_box	= (*m_energy_pristine_old[ilev]).array(mfi);
			amrex::ParallelFor (box,[=] AMREX_GPU_DEVICE(int i, int j, int k){
				Set::Matrix eps = Numeric::FieldToMatrix(eps_box,i,j,k);
				Set::Matrix sig = model_box(i,j,k,0).DW(eps);
				Numeric::MatrixToField(sig_box,i,j,k,sig);
				material.modeltype.UpdateF0(model_box(i,j,k,0).curr.epsp);
				Set::Scalar energy_temp = material.modeltype.W(eps);
				energy_box(i,j,k,0) = energy_temp > energy_old_box(i,j,k,0) ? energy_temp : energy_old_box(i,j,k,0);
			});
		}
	}
}
