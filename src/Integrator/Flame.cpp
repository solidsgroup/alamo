...
Flame::Parse(Flame& value, IO::ParmParse& pp)
{
...

    {
        ...
        if (value.thermal.on) {
            ...
            pp_query_default("thermal.disperssion2", value.thermal.disperssion2, 920.0); // rho; dispersion variables are use to set the outter field properties for the void grain case.
            pp_query_default("thermal.disperssion3", value.thermal.disperssion3, 2418.19); // cp; dispersion variables are use to set the outter field properties for the void grain case.
...
void Flame::TimeStepComplete(Set::Scalar /*a_time*/, int a_iter)
{
    ...
    if (chamber.on)
    {
        ...
        // chamber.pressure = (chamber.mass / (vol / (chamber.R * chamber.T0)));
        // Util::Message(INFO, "At = ", chamber.At);
        // Util::Message(INFO, "Vol = ", vol);
