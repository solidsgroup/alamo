...
Flame::Flame(IO::ParmParse &pp) : Flame() { pp_queryclass(*this); }

// [parser]
void Flame::Parse(Flame &value, IO::ParmParse &pp) {
    BL_PROFILE("Integrator::Flame::Flame()");
    {
        ...
        pp.select<IC::Laminate,IC::Constant,IC::Expression,IC::BMP,IC::PNG>("pf.eta.ic",value.ic_eta,value.geom); // IC
...

    {
        ...
        if (value.thermal.on) {
            ...
            pp_query_required(
                "thermal.m_ap",
                value.thermal
                    .m_ap); // AP Pre-exponential factor for Arrhenius Law
            pp_query_required(
                "thermal.m_htpb",
                value.thermal
                    .m_htpb); // HTPB Pre-exponential factor for Arrhenius Law
            pp_query_required(
                "thermal.E_ap",
                value.thermal.E_ap); // AP Activation Energy for Arrhenius Law
            pp_query_required(
                "thermal.E_htpb",
                value.thermal
            ...
            pp.select_default<IC::Constant, IC::Expression>(
                "laser.ic", value.ic_laser, value.geom);

            // thermal initial condition
            pp.select_default<IC::Constant, IC::Expression, IC::BMP, IC::PNG>(
                "temp.ic", value.thermal.ic_temp, value.geom);
        }
    }

    {
        ...
        if (value.thermal.on) {
            pp_query_required(
                "pressure.a1",
                value.pressure.arrhenius
                    .a1); // Surgate heat flux model paramater - AP
            pp_query_required(
                "pressure.a2",
                value.pressure.arrhenius
                    .a2); // Surgate heat flux model paramater - HTPB
            pp_query_required(
                "pressure.a3",
                value.pressure.arrhenius
                    .a3); // Surgate heat flux model paramater - Total
            pp_query_required(
                "pressure.b1",
                value.pressure.arrhenius
                    .b1); // Surgate heat flux model paramater - AP
            pp_query_required(
                "pressure.b2",
                value.pressure.arrhenius
                    .b2); // Surgate heat flux model paramater - HTPB
            pp_query_required(
                "pressure.b3",
                value.pressure.arrhenius
                    .b3); // Surgate heat flux model paramater - Total
            pp_query_required(
                "pressure.c1",
                value.pressure.arrhenius
                    .c1); // Surgate heat flux model paramater - Total
            pp_query_default(
                "pressure.mob_ap", value.pressure.arrhenius.mob_ap,
                0); // Whether to include pressure to the arrhenius law
            pp_query_default(
                "pressure.dependency", value.pressure.arrhenius.dependency,
                1); // Whether to use pressure to determined the reference Zeta
            pp_query_default(
                "pressure.h1", value.pressure.arrhenius.h1,
                1.81); // Surgate heat flux model paramater - Homogenized
            pp_query_default(
                "pressure.h2", value.pressure.arrhenius.h2,
                1.34); // Surgate heat flux model paramater - Homogenized

        } else {
            pp_query_required(
                "pressure.r_ap",
                value.pressure.power
                    .r_ap); // AP power pressure law parameter (r*P^n)
            pp_query_required(
                "pressure.r_htpb",
                value.pressure.power
                    .r_htpb); // HTPB power pressure law parameter (r*P^n)
            pp_query_required(
                "pressure.r_comb",
                value.pressure.power
                    .r_comb); // AP/HTPB power pressure law parameter (r*P^n)
            pp_query_required(
                "pressure.n_ap",
                value.pressure.power
                    .n_ap); // AP power pressure law parameter (r*P^n)
            pp_query_required(
                "pressure.n_htpb",
                value.pressure.power
                    .n_htpb); // HTPB power pressure law parameter (r*P^n)
            pp_query_required(
                "pressure.n_comb",
                value.pressure.power
...

    {
        ...
        if (phi_ic_type == "psread") {
            ...
            value.ic_phi =
                new IC::Expression(value.geom, pp, "phi.ic.expression");
            // value.ic_phicell = new IC::Expression(value.geom, pp,
            // "phi.ic.expression");
            pp_query_default(
                "phi.zeta_0", value.zeta_0,
                1.0e-5); // Reference interface length for heat integration
    ...
    if (value.m_type != Type::Disable) {
        ...
        pp_query_default(
            "Tref", value.elastic.Tref,
            300.0); // Initial temperature for thermal expansion computation
        pp.queryclass<Model::Solid::Finite::NeoHookeanPredeformed>(
            "model_ap", value.elastic.model_ap);
        pp.queryclass<Model::Solid::Finite::NeoHookeanPredeformed>(
            "model_htpb", value.elastic.model_htpb);
...
void Flame::Advance(int lev, Set::Scalar time, Set::Scalar dt) {
    ...
    if (true) // lev == finest_level) //(true)
    {
        if (thermal.on) {
            ...
            Numeric::Function::Polynomial<4> w(
                pf.w0, 0.0, -5.0 * pf.w1 + 16.0 * pf.w12 - 11.0 * pf.w0,
            ...
            for (amrex::MFIter mfi(*eta_mf[lev], true); mfi.isValid(); ++mfi) {
                ...
                // Phase fields
                amrex::Array4<Set::Scalar> const &etanew =
                ...
                // amrex::Array4<const Set::Scalar> const& phicell =
                // (*phicell_mf[lev]).array(mfi);
                //  Heat transfer fields
                amrex::Array4<Set::Scalar> const &tempnew =
                    (*temp_mf[lev]).array(mfi);
                amrex::Array4<const Set::Scalar> const &temp =
                    (*temp_old_mf[lev]).array(mfi);
                amrex::Array4<Set::Scalar> const &alpha =
                    (*alpha_mf[lev]).array(mfi);
                amrex::Array4<Set::Scalar> const &tempsnew =
                    (*temps_mf[lev]).array(mfi);
                amrex::Array4<Set::Scalar> const &temps =
                    (*temps_old_mf[lev]).array(mfi);
                amrex::Array4<Set::Scalar> const &laser =
                    (*laser_mf[lev]).array(mfi);
                // Diagnostic fields
                amrex::Array4<Set::Scalar> const &mob =
                    (*mob_mf[lev]).array(mfi);
                amrex::Array4<Set::Scalar> const &mdot =
                    (*mdot_mf[lev]).array(mfi);
                amrex::Array4<Set::Scalar> const &heatflux =
                ...
                Set::Scalar zeta_1;
                if (pressure.arrhenius.dependency == 1)
                    zeta_1 = zeta_2;
                else
                    zeta_1 = zeta_0;
                ...
                amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j,
                                                            int k) {
                    Set::Scalar phi_avg =
                        Numeric::Interpolate::NodeToCellAverage(phi, i, j, k,
                                                                0);
                    Set::Scalar eta_lap =
                        Numeric::Laplacian(eta, i, j, k, 0, DX);
                    Set::Scalar K; // Calculate effective thermal conductivity
                    Set::Scalar
                        rho; // No special interface mixure rule is needed here.
                    Set::Scalar cp;
                    ...
                    alpha(i, j, k) =
                        K / rho /
                        cp; // Calculate thermal diffusivity and store in fiel
                    mdot(i, j, k) = rho * fabs(eta(i, j, k) - etanew(i, j, k)) /
                                    dt; // deta/dt
                ...
                amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j,
                    ...
                    Set::Vector grad_eta =
                        Numeric::Gradient(eta, i, j, k, 0, DX);
                    Set::Vector grad_temp =
                        Numeric::Gradient(temp, i, j, k, 0, DX);
                    Set::Scalar lap_temp =
                        Numeric::Laplacian(temp, i, j, k, 0, DX);
                    Set::Scalar grad_eta_mag = grad_eta.lpNorm<2>();
                    Set::Vector grad_alpha =
                        Numeric::Gradient(alpha, i, j, k, 0, DX, sten);
                    Set::Scalar dTdt = 0.0;
                    ...
                    Set::Scalar Tsolid;
                    Tsolid = dTdt + temps(i, j, k) *
                                        (etanew(i, j, k) - eta(i, j, k)) / dt;
                    tempsnew(i, j, k) = temps(i, j, k) + dt * Tsolid;
                    tempnew(i, j, k) =
                        etanew(i, j, k) * tempsnew(i, j, k) +
                ...
                amrex::ParallelFor(
                    bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                        Set::Scalar phi_avg =
                            Numeric::Interpolate::NodeToCellAverage(phi, i, j,
                                                                    k, 0);
                        // phi_avg = phicell(i,j,k);

                        Set::Scalar L;
                        if (pressure.arrhenius.mob_ap == 1)
                            L = thermal.m_ap * pressure.P *
            ...
            for (amrex::MFIter mfi(*eta_mf[lev], true); mfi.isValid(); ++mfi) {
                ...
                amrex::Array4<Set::Scalar> const &eta =
                    (*eta_old_mf[lev]).array(mfi);
                amrex::Array4<Set::Scalar> const &phi =
                    (*phi_mf[lev]).array(mfi);

                Set::Scalar fmod_ap =
                    pressure.power.r_ap * pow(pressure.P, pressure.power.n_ap);
                Set::Scalar fmod_htpb = pressure.power.r_htpb *
                                        pow(pressure.P, pressure.power.n_htpb);
                Set::Scalar fmod_comb = pressure.power.r_comb *
                    ...
                    bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                        ...
                        if (homogeneousSystem == 1) {
                            Set::Vector grad_eta =
                                Numeric::Gradient(eta, i, j, k, 0, DX);
                            Set::Scalar angle =
                                acos(grad_eta[0] / grad_eta.lpNorm<2>()) * 180 /
                                3.1415;
                            if (angle > 90)
                                angle = angle - 90.0;
                            if (angle > 180)
                            ...
                            if (angle > 270)
                                angle = angle - 270.0;
                            L = pressure.power.a_fit +
                                pressure.power.b_fit *
                                    exp(-pressure.power.c_fit * angle);
                        } else {
                            Set::Scalar fs_actual;
                            fs_actual = fmod_ap * phi(i, j, k) +
                                        fmod_htpb * (1.0 - phi(i, j, k)) +
                                        4.0 * fmod_comb * phi(i, j, k) *
                                            (1.0 - phi(i, j, k));
...
void Flame::TagCellsForRefinement(int lev, amrex::TagBoxArray &a_tags,
    ...
    const Set::Scalar *DX = geom[lev].CellSize();
    Set::Scalar dr =
        sqrt(AMREX_D_TERM(DX[0] * DX[0], +DX[1] * DX[1], +DX[2] * DX[2]));
...
void Flame::Integrate(int amrlev, Set::Scalar /*time*/, int /*step*/,
    ...
    // time dependent pressure data from experimenta -> p = 0.0954521220950523 *
    // exp(15.289993148880678 * t)
}
} // namespace Integrator
