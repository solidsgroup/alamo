       ...
       for (int j = bx.loVect()[1]; j<=bx.hiVect()[1]; j++)
	 {
	   ...
	   for (int m = 0; m < number_of_grains; m++)
	       ...
	       for (int n = 0; n < number_of_grains; n++)
		 {
	       ...
	       amrex::Real grad12 = grad12_normal;//grad12_rotated; //0.5*grad12_normal+0.25*(grad11-grad22);//
	       
	       //Second curvature variational terms
	       amrex::Real grad1111=(1/((dx[0]*dx[0]*dx[0]*dx[0])))*
		 ((old_phi(amrex::IntVect(i+2,j),m))-4*(old_phi(amrex::IntVect(i+1,j),m))+6*(old_phi(amrex::IntVect(i,j),m))
	     -4*(old_phi(amrex::IntVect(i-1,j),m))+(old_phi(amrex::IntVect(i-2,j),m)));

               amrex::Real grad2222=(1/((dx[1]*dx[1]*dx[1]*dx[1])))*
		 ((old_phi(amrex::IntVect(i,j+2),m))-4*(old_phi(amrex::IntVect(i,j+1),m))+6*(old_phi(amrex::IntVect(i,j),m))
		  -4*(old_phi(amrex::IntVect(i,j-1),m))+(old_phi(amrex::IntVect(i,j-2),m)));
	       
               amrex::Real grad1112= (1/(24*dx[0]*dx[0]*dx[0]*dx[1]))*
		 ((-old_phi(amrex::IntVect(i+2,j+2),m)+8*old_phi(amrex::IntVect(i+2,j+1),m)
		   -8*old_phi(amrex::IntVect(i+2,j-1),m)+old_phi(amrex::IntVect(i+2,j-2),m))
		  -2*(-old_phi(amrex::IntVect(i+1,j+2),m)+8*old_phi(amrex::IntVect(i+1,j+1),m)
		      -8*old_phi(amrex::IntVect(i+1,j-1),m)+old_phi(amrex::IntVect(i+1,j-2),m))
		  +2*(-old_phi(amrex::IntVect(i-1,j+2),m)+8*old_phi(amrex::IntVect(i-1,j+1),m)
		      -8*old_phi(amrex::IntVect(i-1,j-1),m)+old_phi(amrex::IntVect(i-1,j-2),m))
		  -(-old_phi(amrex::IntVect(i-2,j+2),m)+8*old_phi(amrex::IntVect(i-2,j+1),m)
		    -8*old_phi(amrex::IntVect(i-2,j-1),m)+old_phi(amrex::IntVect(i-2,j-2),m)));
		...
		amrex::Real grad1222= (1/(24*dx[0]*dx[1]*dx[1]*dx[1]))*
		  ((-old_phi(amrex::IntVect(i+2,j+2),m)+8*old_phi(amrex::IntVect(i+1,j+2),m)
		    -8*old_phi(amrex::IntVect(i-1,j+2),m)+old_phi(amrex::IntVect(i-2,j+2),m))
		   -2*(-old_phi(amrex::IntVect(i+2,j+1),m)+8*old_phi(amrex::IntVect(i+1,j+1),m)
		       -8*old_phi(amrex::IntVect(i-1,j+1),m)+old_phi(amrex::IntVect(i-2,j+1),m))
		   +2*(-old_phi(amrex::IntVect(i+2,j-1),m)+8*old_phi(amrex::IntVect(i+1,j-1),m)
		       -8*old_phi(amrex::IntVect(i-1,j-1),m)+old_phi(amrex::IntVect(i-2,j-1),m))
		   -(-old_phi(amrex::IntVect(i+2,j-2),m)+8*old_phi(amrex::IntVect(i+1,j-2),m)
		     -8*old_phi(amrex::IntVect(i-1,j-2),m)+old_phi(amrex::IntVect(i-2,j-2),m)));
	       ...
	       amrex::Real grad1122= (1/(144*dx[0]*dx[0]*dx[1]*dx[1]))*
	        ...
	        +16*(-old_phi(amrex::IntVect(i+2,j+1),m)+16*old_phi(amrex::IntVect(i+1,j+1),m)-30*old_phi(amrex::IntVect(i,j+1),m)
	          +16*old_phi(amrex::IntVect(i-1,j+1),m)-old_phi(amrex::IntVect(i-2,j+1),m))
	        -30*(-old_phi(amrex::IntVect(i+2,j),m)+16*old_phi(amrex::IntVect(i+1,j),m)-30*old_phi(amrex::IntVect(i,j),m)
	          +16*old_phi(amrex::IntVect(i-1,j),m)-old_phi(amrex::IntVect(i-2,j),m))
	        +16*(-old_phi(amrex::IntVect(i+2,j-1),m)+16*old_phi(amrex::IntVect(i+1,j-1),m)-30*old_phi(amrex::IntVect(i,j-1),m)
	          +16*old_phi(amrex::IntVect(i-1,j-1),m)-old_phi(amrex::IntVect(i-2,j-1),m))
	        -(-old_phi(amrex::IntVect(i+2,j-2),m)+16*old_phi(amrex::IntVect(i+1,j-2),m)-30*old_phi(amrex::IntVect(i,j-2),m)
	          +16*old_phi(amrex::IntVect(i-1,j-2),m)-old_phi(amrex::IntVect(i-2,j-2),m)));
