# Calibrating the AP regression rate against experimental r(P)

Fits **one** `(pre_exponential, activation_temperature)` pair for
`Model::PhaseField::AllenCahn` to the whole experimental AP
regression-rate-vs-pressure curve in `AP_reg_rate.csv` (1.0-6.0 MPa), not a
