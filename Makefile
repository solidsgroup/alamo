...
INPUT_BUILDER_EXE ?= $(filter-out bin/test-$(POSTFIX),$(EXE))
INPUT_BUILDER_NAMES = $(patsubst %-$(POSTFIX),%,$(notdir $(INPUT_BUILDER_EXE)))
...
DOC_INPUT_BUILDER_FRAGMENT ?= docs/source/Builder.html
DOC_INPUT_COVERAGE_REPORT ?= $(DOC_INPUT_SCHEMA_DIR)/input-scrape-coverage.json
DOC_INPUT_SCHEMAS = $(addprefix $(DOC_INPUT_SCHEMA_DIR)/,$(addsuffix .schema.json,$(INPUT_BUILDER_NAMES)))
DOC_INPUT_BUILDERS = $(addprefix $(DOC_INPUT_BUILDER_DIR)/,$(addsuffix .html,$(INPUT_BUILDER_NAMES)))
...
default: $(DEP) $(EXE) $(DOC_INPUT_BUILDER_FRAGMENT)
...
.PHONY: .FORCE input-builders docs-input-builders input-scrape-check

docs: docs/build/html/index.html .FORCE
	@printf "$(B_ON)$(FG_MAGENTA)DOCS$(RESET) Done\n" 

docs/build/html/index.html: $(shell find docs/source/ -type f) README.rst $(DOC_INPUT_BUILDER_FRAGMENT) .FORCE
	@printf "$(B_ON)$(FG_MAGENTA)DOCS$(RESET) Generating sphinx\n" 	
	@make -C docs html # > /dev/null

input-builders: docs-input-builders

docs-input-builders: $(DOC_INPUT_BUILDER_FRAGMENT)
	@printf "$(B_ON)$(FG_MAGENTA)INPUT BUILDERS$(RESET) Done\n"

input-scrape-check: $(DOC_INPUT_COVERAGE_REPORT)

$(DOC_INPUT_SCHEMA_DIR)/%.schema.json: bin/%-$(POSTFIX) $(INPUT_BUILDER_ANNOTATE_SCRIPT)
...
$(DOC_INPUT_BUILDER_DIR)/%.html: $(DOC_INPUT_SCHEMA_DIR)/%.schema.json $(INPUT_BUILDER_HTML_SCRIPT) $(INPUT_BUILDER_HTML_TEMPLATE)
	@printf "$(B_ON)$(FG_MAGENTA)INPUT BUILDER$(RESET) $*\n"
	@mkdir -p "$(DOC_INPUT_BUILDER_DIR)"
	@python3 "$(INPUT_BUILDER_HTML_SCRIPT)" --schema "$<" --output "$@.tmp" --title "Alamo Input Builder: $*" > /dev/null
	@mv "$@.tmp" "$@"

$(DOC_INPUT_COVERAGE_REPORT): $(DOC_INPUT_SCHEMAS) $(INPUT_BUILDER_CHECK_SCRIPT) Makefile
...
$(DOC_INPUT_BUILDER_FRAGMENT): $(DOC_INPUT_BUILDERS) $(DOC_INPUT_COVERAGE_REPORT) Makefile
	@printf "$(B_ON)$(FG_MAGENTA)INPUT BUILDERS$(RESET) Updating index\n"
	@mkdir -p "$(dir $@)"
	@set -e; \
	tmp="$@.tmp"; \
	{ \
		printf '<!-- Generated by make docs-input-builders. Do not edit by hand. -->\n'; \
		printf '<div class="input-builder-index">\n'; \
		printf '  <p>Generated for the current Alamo build configuration: <code>$(POSTFIX)</code>.</p>\n'; \
		printf '  <p>Scrape coverage report: <code>_static/input-schemas/$(notdir $(DOC_INPUT_COVERAGE_REPORT))</code>.</p>\n'; \
		printf '  <ul>\n'; \
	} > "$$tmp"; \
	for name in $(INPUT_BUILDER_NAMES); do \
		printf '    <li><a href="_static/input-builders/%s.html">%s</a> <span class="text-muted">(schema: <code>_static/input-schemas/%s.schema.json</code>)</span></li>\n' "$$name" "$$name" "$$name" >> "$$tmp"; \
	done; \
	{ \
		printf '  </ul>\n'; \
		printf '</div>\n'; \
