#!/usr/bin/env python3
"""Audit a pure-binder fit stage and optionally freeze it before the sweep."""
import argparse
import json
...
def main():
    ...
    assert len(rows) == 2
    assert {(r["q_cal_cm2_s"], r["ap_volume_fraction"]) for r in rows} == {(200., 0.), (1000., 0.)}
    evidence = audit(rows, parameters)
    (args.summary.parent / "execution_audit.json").write_text(json.dumps(evidence, indent=2)+"\n")
    assert max(abs(r["late_speed_drift_percent"]) for r in rows) < .2
    for r in rows:
        print(f"{r['name']}: {r['r_cm_s']:.9f} cm/s; error {r['error_figure4_solid_percent']:+.6f}%; "
              f"drift {r['late_speed_drift_percent']:+.6f}%; Ts {r['surface_temperature_K']:.3f} K")
    if args.freeze:
        ...
        (HERE / "freeze_record.json").write_text(json.dumps(dict(
            ...
            criteria=dict(max_endpoint_error_percent=1., max_late_drift_percent=.2,
                          minimum_surface_temperature_K=parameters["T0_K"]),
